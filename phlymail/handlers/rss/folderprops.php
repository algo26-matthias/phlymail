 ...
 * @version 4.1.2 2015-04-01 $Id: folderprops.php 2731 2013-03-25 13:24:16Z mso $
 */
...
$choices = (isset($_PM_['rss']) && $_PM_['rss']) ? $_PM_['rss'] : array();
...
if (isset($_REQUEST['save']) && $_REQUEST['save']) {/*
    ...
    $GlChFile['rss']['show_fields'] = $showfields;
    $GlChFile['rss']['use_preview'] = (isset($_REQUEST['show_preview']) && $_REQUEST['show_preview']);
    $GlChFile['rss']['use_default_fields'] = (isset($_REQUEST['view_default']) && $_REQUEST['view_default']);
    if (0 == $fieldcount) $GlChFile['rss']['use_default_fields'] = true;
    $GlChFile['rss']['orderby'] = isset($_REQUEST['orderby']) ? $_REQUEST['orderby'] : 'lastname';
    $GlChFile['rss']['orderdir'] = isset($_REQUEST['orderdir']) ? $_REQUEST['orderdir'] : 'ASC';
    $DB->set_usr_choices($_SESSION['phM_uid'], $GlChFile);
    $choices = $GlChFile['rss'];
    */
    if (isset($_REQUEST['show_in_sync']) && $_REQUEST['show_in_sync']) {
        $fSet->foldersetting_del('rss', $fid, $_SESSION['phM_uid'], 'not_in_sync');
    } else {
        $fSet->foldersetting_set('rss', $fid, $_SESSION['phM_uid'], 'not_in_sync', 1);
    }
...
$data['big_icon'] = $icon_path.'rss_big.gif';
$data['icon'] = $icon_path.'rss.png';
...
$choices['not_in_sync'] = $fSet->foldersetting_get('rss', $fid, $_SESSION['phM_uid'], 'not_in_sync');
...
foreach (array
        ('number_feeds' => array
                ('type' => 'int'
                ,'method' => 'quota_feedsnum'
                ,'name' => $WP_msg['QuotaNumberFeeds']
                )
        ,'number_groups' => array
                ('type' => 'int'
    ...
    $v['limit'] = $DB->quota_get($_SESSION['phM_uid'], 'rss', $k);
...
$tpl->assign(array
        ...
        ,'form_target' => htmlspecialchars(PHP_SELF.'?'.give_passthrough(1).'&l=folderprops&h=rss&save=1')
        ));

...
* @version 0.3.6 2015-04-06 
* @todo - Create a non-encoded account to use for POP3 (Make APOP possible)
*/
...
$pid = pcntl_fork();
if (-1 == $pid) {
    die('Could not fork to background, exitting'.LF);
} elseif ($pid != 0) {
    die('OK'.LF);
}
$pid_file = $_PM_['path']['conf'].'/services.pop3d.pid';
// Create the PID file to indicate we are running, Leave our PID in it
...
function handle_signal($signo) {
    ...
    case SIGTERM:
        if ($parent === 0) {
            // we are the parent
            // echo "[parent] Caught SIGTERM: Closing down".CRLF;
            foreach ($children as $key => $pid) {
                echo $key . "\n";
                posix_kill($key, SIGKILL);
                $child = pcntl_waitpid(0, $status);
                if ($child > 0) {
                    // echo "[parent] Cleaning up Child - " . $child . CRLF;
...
class pop3_server {
    ...
    public function __construct($socket, $_PM_)
    {
        ...
        $this->msg_queue = msg_get_queue(ftok(__FILE__, 'd'), 0600);
    ...
    {
        declare ( ticks = 1 );
        msg_send($this->msg_queue, 1, $GLOBALS['me'].' WAIT '.time(), false, false);
        $command = $this->socket_get();
        msg_send($this->msg_queue, 1, $GLOBALS['me'].' WORK '.time(), false, false);
    ...
    {
        if ($this->use_APOP) {
            $this->apop_greeting = ' <'.basics::strong_uniqid().'@'.$this->domainname.'>';
        }
...
if (!$listen =& socket_create_listen($_PM_['phlymta']['pop3d_port'])) {
    echo 'Failed to create Listen Socket'.LF;
    exit;
} else {
    echo 'Listening on port: '.$_PM_['phlymta']['pop3d_port'].LF;
}

$msg_queue = msg_get_queue(ftok(__FILE__, 'd'), 0600);

$time = time();
while (true) {
    // Read in the msg_queue
    if ($msg_queue && msg_receive($msg_queue, 0, $type, 32, $message, false, (MSG_IPC_NOWAIT))) {
        echo '[parent] Got MSG: '.$message.LF;
        $message = explode(' ', $message);
        if (isset($children[$message[0]])) {
            if (in_array($message[1], array('WORK', 'WAIT', 'EXIT'))) {
                $children[$message[0]]['state'] = strtolower($message[1]);
                $children[$message[0]]['time'] = isset($message[2]) ? $message[2] : time();
            }
        }
    }

    // Periodically look for PID file, check for dead or timed out client processes
    if (time() > $time) {
        if (!file_exists($pid_file)) {
            // echo '[parent] PID file is missing - assuming controlled shutdown'.LF;
            handle_signal(SIGTERM);
            break;
        }
        clearstatcache();
        foreach ($children as $pid => $data) {
            if ('wait' == $data['state'] && ($data['time'] + 50) < time()) {
                $data['state'] = 'exit';
            }
            if ('exit' == $data['state']) {
                posix_kill($pid, SIGTERM);
                $child = pcntl_waitpid(0, $status, WNOHANG);
                if ($child > 0) {
                    // echo '[parent] Cleaning Up Child - '.$child.LF;
                    unset ($children[$child]);
                }
            }
        }
    ...
    if (@socket_select($read = array($listen), $set_w = NULL, $set_e = NULL, 0, 0) > 0) {
        if ($read[0] == $listen) {
            $ch = socket_accept($listen);
            if (sizeof($children) > $_PM_['phlymta']['pop3d_maxchilds']) {
                socket_write($ch, '-ERR Too many open connections'.CRLF);
                socket_close($ch);
                continue;
            }
            $pid = pcntl_fork();
            if ($pid == -1) {
                ...
                $children[$pid] = array('state' => 'auth', 'time' => time());
                socket_close($ch);
            } else {
                socket_close($listen);

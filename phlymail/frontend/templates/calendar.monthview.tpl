...
function edit_event(evt_id, start_d)
{
    window.open
            ('{edit_evt_link}' + (evt_id ? '&eid=' + evtdata[evt_id].eid : '&gid={gid}')
                    + (start_d ? '&ref_date=' + encodeURIComponent(start_d) : '')
...
function eventdetails(eid)
{
    ...
    var ori = $('#shownevent_' + eid);

    var HTML = '<div id="evtdet_' + eid + '" class="cal_evt_popup ' + ori.attr('class').replace(/ cal_evt_ispast$/, '') + '">';

    if (evtdata[eid]['colour'].length > 0 || evtdata[eid]['repeats'] == 1 || evtdata[eid]['alarm'] == 1) {
        HTML += '<div class="cal_evt_icons">';
        if (evtdata[eid]['colour'].length > 0) {
            HTML += '<div class="cal_evt_colourflag cal_evt_colour_' + evtdata[eid]['colour'] + '"></div>';
        }
        if (evtdata[eid]['alarm'] == 1) {
            HTML += '<img src="{theme_path}/icons/cal_alarm.gif" alt="" />';
        }
        if (evtdata[eid]['repeats'] == 1) {
            HTML += '<img src="{theme_path}/icons/cal_repeating.gif" alt="" />';
        }
        HTML += '</div>';
    }
    ...
    if (evtdata[eid]['title'].length > 0) {
        HTML += '<tr><td class="l t"><strong>{msg_title}:</strong></td><td class="l t">' + evtdata[eid]['title'] + '</td></tr>';
    }
    HTML += '<tr><td class="l t"><strong>{msg_starts}:</strong></td><td class="l t">' + evtdata[eid]['starts'] + '</td></tr>';
    HTML += '<tr><td class="l t"><strong>{msg_ends}:</strong></td><td class="l t">' + evtdata[eid]['ends'] + '</td></tr>';
    HTML += '<tr><td class="l t"><strong>%h%CalDuration%:</strong></td><td class="l t">' + showDuration(evtdata[eid]) + '</td></tr>';
    if (evtdata[eid]['loc'].length > 0) {
        HTML += '<tr><td class="l t"><strong>{msg_location}:</strong></td><td class="l t">' + evtdata[eid]['loc'] + '</td></tr>';
    }
    if (evtdata[eid]['desc'].length > 0) {
        HTML += '<tr><td class="l t"><strong>{msg_description}:</strong></td><td class="l t">' + evtdata[eid]['desc'].replace(/\n/g, '<br />') + '</td></tr>';
    }
    HTML += '</tbody></table></div>';

    $(HTML).css({'left' : ori.css('left'), 'top' : ori.css('top')})
        .bind('dblclick', function (e) { edit_event(this.id.replace(/^evtdet_/, '')); } )
        .bind('mouseout', function (e) { evtdetTO = window.setTimeout('evtdet_close(' + this.id.replace(/^evtdet_/, '') + ')', 750); } )
        .bind('mouseover', function (e) { if (evtdetTO) window.clearTimeout(evtdetTO); } )
        .appendTo('#outline');

    var Me = $('#evtdet_' + eid);
    var myPos = Me.position();
    if (fullh < (myPos.top + Me.height() ) ) {
        Me.css('top', (myPos.top + ( fullh - ( myPos.top + Me.height() ) ) ) + 'px');
    }
    if (fullw < (myPos.left + Me.width() ) ) {
        Me.css('left', (myPos.left + ( fullw - ( myPos.left + Me.width() ) ) ) + 'px');
    }
...
function resize_page()
{
    ...
    try {
        ...
        // This happens, when the iframe is opened outside phlyMail
    }
    show_event(evt);
    events_loaded = 1;
}
...
function show_event(data)
{
    ...
    for (var i in canvtbl.childNodes) {
        if (canvtbl.childNodes[i].nodeName == 'TR') {
            ...
            if (canvtbl.childNodes[i].id && canvtbl.childNodes[i].id == 'days_heading') {
                for (var j in canvtbl.childNodes[i].childNodes) {
                    if (canvtbl.childNodes[i].childNodes[j].nodeName == 'TD') {
                        wdline_height = canvtbl.childNodes[i].childNodes[j].offsetHeight;
                        break;
                    }
                }
            }
            if (canvtbl.childNodes[i].className && canvtbl.childNodes[i].className == 'monthline') {
                monatszeilen++;
            }
        }
    }
    hh_realh = hh_height = ((canvash - (mhead_height+wdline_height)) / monatszeilen) - 1;
    hh_height = Math.floor(hh_height);
    hh_realh -= hh_height;
    hh_realw = hh_width = (availw/7) - (msie ? 1 : 0);
    ...
    for (var i in canvtbl.childNodes) {
        ...
        if (canvtbl.childNodes[i].nodeName == 'TR') {
            for (var j in canvtbl.childNodes[i].childNodes) {
                if (canvtbl.childNodes[i].childNodes[j].className) {
                    if (canvtbl.childNodes[i].childNodes[j].className.match(/^cal_mnth_other/) || canvtbl.childNodes[i].childNodes[j].className.match(/^cal_mnth_space/)
                             || canvtbl.childNodes[i].childNodes[j].className.match(/^cal_mnth_curr/) || canvtbl.childNodes[i].childNodes[j].className.match(/^cal_mnth_showday/)) {
                    ...
                    if (canvtbl.childNodes[i].childNodes[j].className == 'cal_mnth_label' || canvtbl.childNodes[i].childNodes[j].className == 'cal_mnth_label.sunday') {
                        canvtbl.childNodes[i].childNodes[j].style.width = hh_width + 'px';
                    }
                }
            }
        }
    ...
    for (var mydatum in data) {
        for (var i in data[mydatum]) {
            ...
            if ((data[mydatum][i]['starth']*60 + data[mydatum][i]['startm']) <= (data[mydatum][(i-1)]['endh']*60 + data[mydatum][(i-1)]['endm'])) {
                ...
                // Termin zurück, der die erste von [i-1]['pos'] abweichende max_pos hat, um zu sehen, ob wir einen dieser Termine
        ...
        for (var i in data[mydatum]) {
            ...
            var start_hi  = my_cell.offsetHeight - 19; // Reduce overall height according toe abovr
...
$(document).ready(function () {
    resize_page();
    draw_tasks();
    $('tr.monthline td').bind('dblclick', function(event) {
        edit_event(0, this.id.replace('td_', ''));
                ...
                <tr id="main_heading">
                    <td colspan="7">
                        <div class="sendmenubut cal_mnth_monthhead">
                            <div style="float:left"><a href="{oneback}"><img src="{theme_path}/icons/nav_left_big.gif" alt="" /></a></div>
                            <div style="float:right;"><a href="{oneforward}"><img src="{theme_path}/icons/nav_right_big.gif" alt="" /></a></div>
                            {month_l}
                        </div>
                    </td>
                </tr>
                <tr id="days_heading">
                    <td class="cal_mnth_label{label_monday}">{monday_l}</td>
                    <td class="cal_mnth_label{label_tuesday}">{tuesday_l}</td>
                    <td class="cal_mnth_label{label_wednesday}">{wednesday_l}</td>
                    <td class="cal_mnth_label{label_thursday}">{thursday_l}</td>
                    <td class="cal_mnth_label{label_friday}">{friday_l}</td>
                    <td class="cal_mnth_label{label_saturday}">{saturday_l}</td>
                    <td class="cal_mnth_label{label_sunday}">{sunday_l}</td>
                </tr><!-- START mnth_weekline -->
                <tr class="monthline">{mnth_weekday}</tr><!-- END mnth_weekline -->
            </tbody>
        </table><!-- START mnth_daycell -->
                <td class="{dayclass}{sunday}" title="{title}" id="td_{datelong}"><div class="cal_mnth_date" id="draw_{day}">{holiday}{date}</div></td><!-- END mnth_daycell --><!-- START mnth_kw -->
                <div class="cal_mnth_kw">{kw}</div><!-- END mnth_kw -->
    </div>
    <div style="float:right;">
        <table border="0" cellpadding="2" cellspacing="0" id="overview">
            <tr>

<div data-role="page" data-add-back-btn="true" data-back-btn-iconpos="notext" data-back-btn-text="%h%CoreBack%">
    <div data-role="header" data-position="fixed">
        <h1>{pageTitle}</h1>
        <a href="{PHP_SELF}?{passthru}" data-icon="home" data-iconpos="notext" data-direction="reverse" class="ui-btn-right jqm-home">Home</a>
    </div>
    <div data-role="content">
        <ul data-role="listview"><!-- START line -->
            <li>
                <a href="{link}" data-ajax="false"><!-- START icon -->
                    <img src="{src}" alt="{alt}" class="ui-li-icon"><!-- END icon --><!-- START thumb -->
                    <img src="{src}" alt="{alt}"><!-- END thumb -->
                    <h3><!-- START primary_icon -->
                        <img src="{src}" alt="{alt}"><!-- END primary_icon --><!-- START priority_icon -->
                        <img src="{src}" alt="{alt}"><!-- END priority_icon --><!-- START has_attach -->
                        <span class="ui-icon ui-icon-custom ui-icon-attachment">&nbsp;</span><!-- END has_attach --><!-- START has_colour -->
                        <span class="cmark_square cmark_{colour}"></span><!-- END has_colour -->
                        {primary}
                    </h3><!-- START secondary -->
                    <p><strong>{secondary}</strong></p><!-- END secondary --><!-- START tertiary -->
                    <p>{tertiary}</p><!-- END tertiary --><!-- START aside -->
                    <p class="ui-li-aside"><strong>{aside}</strong></p><!-- END aside -->
                </a>
            </li><!-- END line -->
        </ul><!-- START folderisempty -->
        <h3>%h%CoreFolderIsEmpty%</h3><!-- END folderisempty -->
    </div>
</div>